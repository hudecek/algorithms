...
void calculateOpposites(Node* node){
    ...
    Node* oppositor = NULL;
    if(length % 2 == 0 && isCircular(node)) {
        oppositor = node;
        for(int i=0; i<length/2; i++) {
            oppositor = oppositor->next;
        }
        for(int i=0; i<length/2; i++) {
            oppositor->opposite = node;
            node->opposite = oppositor;
            node = node->next;
            oppositor = oppositor->next;
        }
    } else {
        //fprintf(stderr, "The list's parity is odd");
    }
}
