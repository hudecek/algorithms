...
int isValid(Node* tree, int minimumAllowedValue, int maximumAllowedValue) {
    if(tree == NULL)
        return 1;
    if(tree->nodeValue1 >= minimumAllowedValue && tree->nodeValue2 >= tree->nodeValue1 && tree->nodeValue2 <= maximumAllowedValue) {
        return isValid(tree->leftSubtree, minimumAllowedValue, tree->nodeValue1)*
                isValid(tree->middleSubtree, tree->nodeValue1, tree->nodeValue2)*
                isValid(tree->rightSubtree, tree->nodeValue2, maximumAllowedValue);
    }
    return 0;
}
